    ...
    socket.onmessage = (e: MessageEvent<string>) => {
      ...
      if (msg.type === 'session_event') {
        ...
        if (eventType === 'message_start') {
          ...
          setInProgressMessages(prev => ({
            ...prev,
            [target]: { role, content: initialContent }
          }));
          ...
          setInProgressMessages(prev => {
            const current = prev[target] || { role: msgObj?.role || 'assistant', content: [] };
        ...
        // Ignore noisy events (agent_start, turn_start, turn_end, tool_execution_*, irc_message, notice, etc.)
      }
      if (msg.type === 'error') {
